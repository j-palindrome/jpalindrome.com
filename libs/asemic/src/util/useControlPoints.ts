...
function useControlPointArray(group: GroupBuilder, builder: BrushBuilder<any>) {
  ...
  const reload = () => {
    for (let i = 0; i < builder.settings.maxCurves; i++) {
      controlPointCounts.array[i] = group.curves[i].length
    }
...
export function usePoints(group: GroupBuilder, builder: BrushBuilder<any>) {
  const { curvePositionArray, curveColorArray, hooks } = useControlPointArray(
    group,
    builder,
  )
...
export function useCurve(group: GroupBuilder, builder: BrushBuilder<any>) {
  ...
  const { curvePositionArray, curveColorArray, controlPointCounts, hooks } =
    useControlPointArray(group, builder)
